...
Output: 2
Explanation: There are 7 ways to rob the houses. The way which leads to minimum capability is to rob the 
house at index 0 and 4. Return max(nums[0], nums[4]) = 2.
 */

public class HouseRobber4 {
        public static void main(String[] args) {
            HouseRobber4 sol = new HouseRobber4();
            System.out.println(sol.minCapability(new int[]{2,3,5,9}, 2));
            System.out.println(sol.minCapability(new int[]{2,7,9,3,1}, 2));
        }
        public int solve(int[] nums , int k , int n , int[][] dp , int index){
         if(k<=0)return 0;
         if(index >= n)return Integer.MAX_VALUE;
         if(dp[index][k]!=-1){
            return dp[index][k];
         }
         int take = Math.max(nums[index], solve(nums , k-1 , n , dp , index+2));
         int nottake = solve(nums , k ,n,dp ,index+1);
         dp[index][k] = Math.min(take , nottake);
         return dp[index][k];
    }
    public int minCapability(int[] nums, int k) {
        ...
        int index = 0;
        return solve(nums , k , n ,dp,index);
    }
}
